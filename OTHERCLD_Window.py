################################################################################
#
# Class OTHERCLD - CLass of attributes and functions that acquires associated 
...
#-----------------------------------------------------------------------
#
# OTHERCLDcallback - call back function when TEST, CANCEL or SCORE buttons
#               are pressed
#
#-----------------------------------------------------------------------
    def OTHERCLDcallback(self, widget, string):

        # Set up the output file depending upon whether this is for score
        # or just a test run
        if (string == "SCORE"):
            NLET_cmd = "/proj/sot/ska/bin/python /data/acis/LoadReviews/script/NONLOADEVENTTRACKER/RecordNonLoadEvent.py OTHERCLD --source NLET "
        else:
            NLET_cmd = "/proj/sot/ska/bin/python /data/acis/LoadReviews/scriptNONLOADEVENTTRACKER//RecordNonLoadEvent.py -t  OTHERCLD --source NLET "
        ...
        if (string == "SCORE") or (string == "TEST"):
            ...
            # Split the comments on carriage returns
            print user_comments.split("\n")
...
#        os.system(NLET_cmd)
        ...
        self.BuildOTHERCLDWindow.destroy()
        return False


    def Pop_UP_OTHERCLD_Window(self):
        # 1111 -  Create the Basic window
        self.BuildOTHERCLDWindow = gtk.Window(gtk.WINDOW_TOPLEVEL)
        # set the title and size of the window.
        self.BuildOTHERCLDWindow.set_title("Some Other CLD File")
        self.BuildOTHERCLDWindow.set_size_request(1000,700)
        self.BuildOTHERCLDWindow.set_border_width(10)
        ...
        self.OTHERCLD_VBox = gtk.VBox(False, 20)
...
#        self.UserComment_entry.set_text("None")
        ...
        self.SCOREbutton.connect("clicked", self.OTHERCLDcallback, "SCORE")
        ...
        self.CANCELbutton.connect("clicked", self.OTHERCLDcallback, "CANCEL")
        ...
        self.TESTbutton.connect("clicked", self.OTHERCLDcallback, "TEST")
        self.TextEntry_Table.attach(self.TESTbutton,
                                    start_col, start_col + 1,
                                    start_row, start_row + 1)

        # 2222 - Add the table to the VBOX
        self.OTHERCLD_VBox.pack_start(self.TextEntry_Table)

        # Add the VBox to the window box
        self.BuildOTHERCLDWindow.add(self.OTHERCLD_VBox)


        # 1111 - Done creating the BuildEVENTTRACKER Window. Show it.
        self.BuildOTHERCLDWindow.show_all()
